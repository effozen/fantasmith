...
사용할 프로그램은 [obsidian-mcp](https://github.com/StevenStavrakis/obsidian-mcp) 입니다.

다양한 MCP 서버가 있지만, 본 플러그인이 가장 다운로드 수가 많으며, 기능이 많은 듯 하여 선택했습니다.

먼저 Claude Desktop을 실행하고 설정 화면으로 이동합니다.

<Figure fileName="step1.webp" alt="Claude Desktop 설정 화면" caption="Claude Desktop 설정 화면" />

Settings(설정) -> Developer(개발자) -> 설정 편집으로 가면 다음과 같은 화면을 확인할 수 있습니다.

<Figure fileName="step2.webp" alt="Claude Desktop 설정 편집 화면" caption="Claude Desktop 설정 편집 화면" />

`claude_desktop_config.json` 파일을 열어서 수정하면 MCP 서버와 쉽게 연결할 수 있습니다.

이 파일을 코드나 문서 편집기로 열어보면 다음과 같은 화면이 뜹니다.

<Figure fileName="step3.webp" alt="Claude Desktop 설정 파일" caption="Claude Desktop 설정 파일" />

이 파일에 이제 MCP 서버의 정보를 추가해주면 됩니다.

MCP 서버 정보를 추가한다...? 라고 하니까 굉장히 어렵게 느껴질 수 있습니다.

하지만 걱정하지 마세요. 이미 만들어진 MCP 서버를 활용할 예정이기에, 설정 파일에 추가할 내용은 간단합니다.

obsidian-mcp 깃허브에 접속해서 보면, 어떻게 사용해야하는 지 설명되어 있습니다.

우리는 시키는대로 사용하면 됩니다. ㅎㅎ.

<Figure
  fileName="step4.webp"
  alt="obsidian-mcp 깃허브"
  caption="obsidian-mcp 깃허브"
  href="https://github.com/StevenStavrakis/obsidian-mcp"
/>

그러면 한번 설정해볼까요? Vault가 있는 Path를 기억해서, 다음과 같이 설정해줍니다.
...
```json
{
  "mcpServers": {
    "obsidian": {
      "command": "npx",
      "args": ["-y", "obsidian-mcp", "/Vault 경로를 넣어주세요."]
    }
  }
}
```

<Figure fileName="step5.webp" alt="obsidian-mcp 설정 예시" caption="obsidian-mcp 설정 예시" />

그리고, 저장하고 Claude Desktop을 재시작합니다.

재시작 후, 다시 설정 화면으로 가면 다음과 같은 화면을 확인할 수 있습니다.

<Figure fileName="step6.webp" alt="Claude Desktop MCP 서버 설정 화면" caption="Claude Desktop MCP 서버 설정 화면" />

<Figure fileName="step7.webp" alt="Claude Desktop MCP 서버 설정 화면" caption="Claude Desktop MCP 서버 설정 화면" />

이제 MCP 서버가 정상적으로 연결된 것을 확인할 수 있습니다.

많은 글들을 보면 MCP 설정을 하면 망치모양이 나온다고 했는데, 패치되어서, 망치모양이 아닌 드로워 아이콘을 클릭해야 보이는 것 같더라구요.

관련해서 MCP 공식 문서에 있는 내용을 첨부하니 참고하시면 좋을 것 같습니다.
...
<Figure fileName="step9.webp" alt="Claude Desktop MCP 서버 테스트" caption="Claude Desktop MCP 서버 테스트" />

위와 같이 권한 요청이 뜨면 성공입니다.

아주 잘 동작하는 것을 확인할 수 있습니다.

<Figure
  fileName="step10.webp"
  alt="Claude Desktop MCP 서버 테스트 결과"
  caption="Claude Desktop MCP 서버 테스트 결과"
/>

## 목표 설정을 위한 프롬프트 엔지니어링 및 MCP Host와 연동

이제 MCP 서버가 연결되었으니, 본격적인 목적 달성을 위한 프롬프트 엔지니어링을 시작해보겠습니다.

사실, 여기서 한가지 고민이 생겼는데, 과연 어떻게 내가 매번 채팅을 입력할 때마다 자동으로 특정 프롬프트를 읽어와서 그걸 바탕으로 원하는 동작을 하게 할 수 있을까? 였습니다.

이와 관련해서 찾아보니 n8n을 이용해서 워크플로를 설게하고 첫 진입점을 Claude Desktop으로 잡고 하는 게 제일 확실한 방법이긴 했는데... 우선은 간단하게 프롬프트만으로 접근해보고자 합니다.

우선, 목표 설정을 위한 프롬프트를 작성해보겠습니다.

```markdown
너는 세계 최고의 자기계발 전문가이자, 코치이자 트레이너야.
나는 더 발전된 사람이 되고 싶어.
실제로 실천할 수 있는 목표를 세우고자 해.

SMART 원칙에 따라서 목표를 설정해줄래?

그리고, 만약 내가 입력한 목표가 터무니없거나, 현실가능성이 없으면 이와 관련해서도 제대로 목표를 세울 수 있게 조언해줘.
```

굉장히 간단해보이지만, 핵심이 잘 담기게 작성했습니다.

회사 프로젝트 등으로 프롬프트를 많이 짜고 연구를 해봤었는데, AI의 자율성을 최대한 보장하면서도 원하는 결과를 얻을 수 있는 간결한 프롬프트가 좋다는 것을 배웠습니다.

그래서 위와 같이 설정해주었고, 이를 바탕으로 돌려보면 다음과 같은 결과물이 나옵니다.

<Figure fileName="step11.webp" alt="목표 설정 테스트 결과" caption="목표 설정 테스트 결과" />

매우매우 잘 나오는 것을 볼 수 있습니다. 사실 생각보다도 더 잘 나와서 놀랐습니다 ㅎㅎ..

이제 이를 통해서 AI와 대화를 하면서 목표를 잡아가고 출력을 받아보면 됩니다.

이때, 출력된 데이터는 추후 다른 기능에서 활용할 수 있도록, 별도의 md 파일로 저장해두고자 합니다.

이걸 위해서 별도의 프롬프트를 작성해보겠습니다.

```markdown
다음과 같은 양식으로 출력해줘.
저장하는 디렉토리는 root의 "test/[목표 명]"이고, 파일 명은 다음과 같아. "yyyy-MM-dd [목표 명]" 이렇게 만들어줘. yyyy-mm-dd는 오늘 날짜야.
...
이렇게 작성한 뒤 진행을 해보았습니다.

<Tabs>
<TabItem value="요청" label="요청" default>
<Figure fileName="step13.webp" alt="MCP 사용 화면" caption="MCP 사용 화면" />

미리 만들어둔 출력 프롬프트를 바탕으로 요청을 해보았습니다.

결과 탭을 눌러서 결과를 확인해보세요!

</TabItem>
<TabItem value="결과" label="결과">
<Figure fileName="step14.webp" alt="MCP 사용 결과" caption="MCP 사용 결과" />
<Figure fileName="step15.webp" alt="MCP 사용 결과" caption="MCP 사용 결과" />
<Figure fileName="step16.webp" alt="MCP 사용 결과" caption="MCP 사용 결과" />

결과를 보니 매우 잘 나온 것을 확인할 수 있습니다.
