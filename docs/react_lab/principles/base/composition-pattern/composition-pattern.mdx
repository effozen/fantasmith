...
title: '합성(Composition pattern)으로 살펴보는 리액트 컴포넌트 설계 핵심'
image: '/img/zen-social-card.jpeg'
tags: [designPattern, principles]
sidebar_position: 1
sidebar_label: '합성(Composition pattern)으로 살펴보는 리액트 컴포넌트 설계 핵심'
keywords: ['Frontend', 'intro', 'composition pattern', '복합체 패턴', '디자인 패턴', 'design pattern']
last_update:
  date: 2024-11-16
  author: zen
---
...
> `Composition Pattern`이 뭔지 알았다.
> `Composition Pattern`이 왜 리액트의 핵심 개발 원리인지 이해했다.
> `Composition Pattern`이 어떻게 리액트에서 활용되는지 파악했다.
...
리액트의 `합성`이라는 개념의 핵심이고, 함수를 바탕으로 한 리액트를 통해 일상적으로 사용했으면서도 크게 와닿지 않았던 것 같다.<br/>

이번에 [복합 컴포넌트 패턴으로 컴포넌트 재사용 높이기](/react_lab/dev-log/component_making/component_composition_pattern_with_header/component_composition_pattern_with_header)와 [Dropdown 컴포넌트를 구현하고 이룬 성장](/react_lab/dev-log/component_making/dropdown_and_growth/dropdown_and_growth)에 적힌 컴포넌트들을 구현하면서 이 개념의 의미가 💡! 하고 받아들여지게 되었다.<br/>
...
import img1 from './img/component_composition_pattern_img1.webp';

<figure>
  <img src={img1} alt="missing" width='60%'/>
  <figcaption>컴포지션 패턴의 하나의 예 : 파일 시스템 구조</figcaption>
</figure>
...
이게 간단한 설명인데, 좀 더 참고하고 싶으면 다음의 글을 보는 것을 추천한다.<br/>
...
위의 글을 통해, 합성 패턴이란 무엇인지 얼추 이해할 수 있었다.<br/>
...
1번은 `class` 문법과 관련된 내용이고, 2, 3, 4번은 `function` 문법에 기반해서 이야기를 할 듯 하다.<br/>

1번은 사실 `class` 문법으로 보여주기 쉬워서 클래스 문법으로 설명하지만, 당연하게도 함수형 문법에도 사용 가능하다. (2번에 해당) <br/>

`JSX`를 사용하는 이상, 2, 3, 4 역시 `class` 문법에 적용될 수 있지만(특히 2번은 1번과 유사), 이들은 `function` 문법에 초점을 둬서 이야기를 하고자 한다.<br/>
...
클래스 문법이 생소한 독자분들이 계실 것 같아, 말해보자면 react의 `class` 문법에서는 `render()`가 `function`의 `return`과 같은 역할이다. <br/>
...
어찌 되었든, HOC을 통해 컴포넌트의 기능을 확장하거나 공통 로직을 재사용할 수 있다.<br/>
...
HOC를 사용하면 Button 컴포넌트에 로깅 기능을 추가할 수 있으며, 다른 컴포넌트에도 동일한 HOC를 적용할 수 있다. <br/>
...
[복합 컴포넌트 패턴으로 컴포넌트 재사용 높이기](/react_lab/dev-log/component_making/component_composition_pattern_with_header/component_composition_pattern_with_header) <br/>

위 글은 내가 `Compound Components`를 구현한 내용을 담고 있다. 이를 통해서 좀 더 구체적인 예시를 살펴볼 수 있다. <br/>
