...
<details>
  ...
  1.
  [naver](https://navermaps.github.io/maps.js.ncp/docs/naver.html)[.maps](https://navermaps.github.io/maps.js.ncp/docs/naver.maps.html).MapSystemProjection**<br/>
  ...
  2.
  [naver](https://navermaps.github.io/maps.js.ncp/docs/naver.html)[.maps](https://navermaps.github.io/maps.js.ncp/docs/naver.maps.html).LatLngBounds**<br/>
...
그러나, 이는 의도대로 동작하지 않았다. `top-child`에는 정상적으로 이벤트가 적용이 되나, `bottom-child`에는 적용이 되지 않았기 때문이다.<br/>

원인은 다음과 같았다.<br/>

스택 컨테스트(`Stacking Context`)에 의한 문제였다.<br/>

간단하게만 설명하면, CSS에서 요소가 화면에 보여지는 `스택 컨텍스트`라는게 존재한다. `z-index`를 걸게 되면, 높은 `z-index`값을 가진 요소는 낮은 값을 가진 요소보다 앞에 위치하게 된다.<br/>
...
## 🤔 캔버스와 지도 간의 미묘한 동기화 차이를 어떻게 할 것인가 ?
